...
;(function( win, $, undefined ){
	...
	function createSwipeTable( $table ){
		...
		var $btns = $( "<div class='tablesaw-advance'></div>" ),
			/*
			$prevBtn = $( "<a href='#' class='tablesaw-nav-btn btn btn-default left'></a>" ).appendTo( $btns ),
			$nextBtn = $( "<a href='#' class='tablesaw-nav-btn btn btn-default right'></a>" ).appendTo( $btns ),
			*/
			$prevBtn = $('.prevBtn'),
			$nextBtn = $('.nextBtn')
			hideBtn = 'disabled',
			persistWidths = 'tablesaw-fix-persist',
			$headerCells = $table.find( "thead tr.header th" ),
			$headerCellsNoPersist = $headerCells.not( '[data-priority="persist"]' ),
			headerWidths = [],
			$head = $( document.head || 'head' ),
			tableId = $table.attr( 'id' ),
			$defaultDisplayColumns = 5,
		...
		function getNext(){
			...
			// console.log($headerCellsNoPersist);
		...
		function fakeBreakpoints() {
			...
			$headerCells.each(function( index ) {
				...
				if( sum > containerWidth || count > $defaultDisplayColumns ) {
					displayedCount++;
					hideColumn( this );
				} else {
					showColumn( this );
				...
				if( displayedCount > $defaultDisplayColumns ){
					$displayColumns = $defaultDisplayColumns;
				} else {
					$displayColumns = displayedCount;
				}
		...
		function advance( fwd ){
			...
			if( canAdvance( pair ) ){
				...
				for( x=0; x<$displayColumns; x++ ){
					if( fwd ){
						hideColumn( $headerCellsNoPersist.get( pair[ 0 ] + x ) );
						showColumn( $headerCellsNoPersist.get( pair[ 1 ] + x ) );					
					} else {
						hideColumn( $headerCellsNoPersist.get( pair[ 0 ] - x ) );
						showColumn( $headerCellsNoPersist.get( pair[ 1 ] - x ) );											
					}
